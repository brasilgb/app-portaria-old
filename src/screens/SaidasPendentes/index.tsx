        ...
        <>
            ...
            <AppLayout bgColor={"bg-solar-gray-dark"}>
                ...
                <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
                    <View className="bg-solar-orange-middle border border-white mx-2 my-2 rounded-lg py-4">
                        <View className="flex-col items-center justify-center py-2">
                            <Text className="text-lg uppercase text-solar-blue-dark font-Poppins_500Medium">Saídas pendentes</Text>
                        </View>
                    ...
                    <View className="mx-2 p-1 bg-white border-gray-300 rounded-md">
                        <View className="bg-solar-gray-dark flex-row items-center justify-between border-y border-x border-gray-300">
                            ...
                            <Text className="w-[124px] px-1 py-3 text-base font-Poppins_500Medium">Data</Text>
                            <Text className="w-16 px-1 py-3 text-base font-Poppins_500Medium">Hora</Text>
                            <Ionicons name="checkmark-circle" size={36} color="#F1F1F1" />
                        </View>
                        {visitasPendentes?.map((mt: any, index: any) => (
                            <View key={mt.cpf + index} className={`${index % 2 ? 'bg-blue-50' : 'bg-red-50'} flex-row items-center justify-between border-b border-x border-gray-300`}>
                                <Ionicons name="alert-circle" size={34} color="#29ABE2"
                                    onPress={() => navigation.navigate('InfoVisitanteEntrada', { data: { ident: mt.ident, nome: mt.nome } })}
                                />
                                ...
                                <View className="w-[124px] px-1">
                                    ...
                                    <View className="flex-row items-center justify-start pb-1 pt-0.5">
                                        ...
                                        <Text onPress={() => handleDatePicker(mt.cpf + index)} className="text-base font-Poppins_400Regular">{newIndex === mt.cpf + index ? date.toLocaleDateString() : moment().format("DD/MM/YYYY")}</Text>
                                    </View>
                                </View>
                                <View className="w-16 px-1">
                                    ...
                                    <Text className="pt-0.5 text-base font-Poppins_400Regular" onPress={() => handleTimePicker(mt.cpf + index)}>{newIndex === mt.cpf + index ? moment(hour).format("HH:mm") : moment().format("HH:mm")}</Text>
                                </View>
                                <Ionicons name="checkmark-circle" size={36} color={`${bColor === mt.ident ? '#e73e3e' : '#27a716'}`}
                                    onPress={() => handleExitVisita(
                                        mt.ident,
                                        newIndex === mt.cpf + index ? moment(date).format("YYYYMMDD") : moment().format("YYYYMMDD"),
                                        newIndex === mt.cpf + index ? moment(hour).format("HHmm") : moment().format("HHmm")
                                    )} />
                            </View>
                        ))}
                    </View>
